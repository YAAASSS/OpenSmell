...
/*
 * OpenSmell experimental Generic ResourceGraph portable conformance verifier.
 *
 ...
 *
 * and independently checks the structural transport rules represented by the
 * experimental Generic ResourceGraph 0.1 JSON Schema.
 *
 ...
 * OpenSmell implementation and it does not perform registry-specific dispatch.
 */
...
const EXPECTED_VECTOR_COUNT = 42;
const EXPECTED_VALID_VECTOR_COUNT = 12;
const EXPECTED_INVALID_VECTOR_COUNT = 30;
const EXPECTED_PRESERVATION_VECTOR_COUNT = 11;
...
/*
 * Generic ResourceGraph transport validation.
 *
 * RFC-0008 deliberately does not require this verifier to know every
 * resource type. A resource only needs:
 *
 *   - a non-empty string "type";
 *   - a non-empty string "id";
 *   - when present, a non-empty string "type_version".
 *
 * All other JSON members are transport extensions and are accepted.
 *
 * Namespace policy for newly registered versioned resource contracts belongs
 * to the resource type registry/specification layer, not this generic
 * transport validator. Therefore an unknown versioned resource can still be
 * transported even if its type identifier is not namespaced.
 */
function validateResource(resource) {
    ...
    if (hasOwn(resource, "type_version")) {
        if (!isNonEmptyString(resource.type_version)) {
            return false;
        }
    }
 ...
 * verifier does not understand. That is essential for RFC-0008 forward
 * compatibility: unknown resource types, unknown type versions and arbitrary
 * resource/document extensions must survive transport.
 */
...
/*
 * Minimal independent JavaScript Generic ResourceGraph parse/serialize path.
 *
 * It intentionally performs no registry dispatch. Unknown JSON members are
 * retained rather than discarded.
 */
function parseGraph(document) {
    if (!validateGraph(document)) {
        throw new Error(
            "Cannot parse invalid Generic ResourceGraph document."
        );
    }
...
function serializeGraph(graph) {
    if (!validateGraph(graph)) {
        throw new Error(
            "Cannot serialize invalid Generic ResourceGraph document."
        );
    }
...
function main() {
    ...
    console.log(
        "SUCCESS: JavaScript agrees with all portable Generic " +
        "ResourceGraph conformance vectors and preserves all " +
        "preservation vectors."
    );
}
